...
class Router(implicit ec: ExecutionContext) extends Directives {
  ...
  private val indexHtml = httpEntity(
    os.read(os.resource / "public" / "index.html"))
  private val webpackBundle = httpEntity(os.read(os.resource / "out-bundle.js"))
  private val webpackSourceMap = httpEntity(
    Try(os.read(os.resource / "out-bundle.js.map")).getOrElse(""))
  ...
  private val apiRouter =
    sloth.Router[ByteBuffer, Future].route[ExampleApi](ExampleApiImpl)
  private val apiRoutes =
    pathPrefix("api")(AkkaHttpRoute.fromFutureRouter(apiRouter))
  private def httpEntity(content: String): HttpEntity.Strict =
